...
class Model implements Iterable<Model.Sq> {
    ...
    /**This initializes the board by looping the solution array and
     *setting sequence numbers for the first and last elements.
     *Additionally, it makes the first and last squares fixed.
     * @param solution which is a 2 dimensional array
     */
    ...
    Model(int[][] solution) {
        ...
        _allSuccessors = successorCells(_width, _height);
        ...
        for (int wi = 0; wi < _width; wi++) {
            for (int he = 0; he < _height; he++) {
                _solnNumToPlace[_solution[wi][he] - 1 ] = pl(wi, he);

                if (_solution[wi][he] == 1) {
                    _board[wi][he] = new Sq(wi, he, 1,
                            true, arrowDirection(wi, he), 0);
                } else if (_solution[wi][he] == _width * _height) {
                    _board[wi][he] = new Sq(wi, he,
                            _width * _height, true, 0, 0);
                } else {
                    _board[wi][he] = new Sq(wi, he, 0,
                            false, arrowDirection(wi, he), -1);
                }
            }
        }
        for (Sq[] col: _board) {
        ...
        for (Sq square: _allSquares) {
            PlaceList [][][] successors = new PlaceList[_width][_height][0];
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        Sq[][] copyboard = new Sq[_width][_height];
        for (int w = 0; w < _width; w = w + 1) {
            for (int h = 0; h < _height; h = h + 1) {
                copyboard[w][h] = new Sq(model._board[w][h]);
                _allSquares.add(copyboard[w][h]);
            }
        }
        for (int w = 0; w < _width; w = w + 1) {
            for (int h = 0; h < _height; h = h + 1) {
                copyboard[w][h]._head = model._board[w][h]._head;
                copyboard[w][h]._predecessor = model._board[w][h]._predecessor;

                copyboard[w][h]._successor = model._board[w][h]._successor; }
        }
        this._board = copyboard;
    }
    ...
    void init(int width, int height) {
        ...
        ArrayList<Sq>  allSquares = new ArrayList<Sq>();
        for (Sq elem: allSquares) {
            elem  = null;
        }
    ...
    void solve() {
        for (Sq[] column: _board) {
            for (Sq square: column) {
                square._sequenceNum = solution()[square.x][square.y];
                square._group = 0;
            }
        }
    ...
    final class Sq {
         ...
         *
         * @param s1 is a square
         * @return Returns a boolean whether a square is connectable
         */
        boolean connectable(Sq s1) {
            if (dirOf(this.x, this.y, s1.x, s1.y) == this._dir) {
         ...
         *
         * @param s1 is a square
         * @return Returns a boolean whether or not a square has been connnected
         *  */
        ...
        boolean connect(Sq s1) {
            if (!connectable(s1)) {
                ...
                this._successor = s1._predecessor;
                s1._predecessor = this._predecessor;
                if (this._sequenceNum != 0
                        && this._sequenceNum != _width * _height) {
                    this._successor._sequenceNum = s1._sequenceNum + 1;
                }
                if (this._sequenceNum != 0 && s1._sequenceNum != 0) {

                    this._head._group = joinGroups(this._group, s1._group);
                }
                if (this._sequenceNum != 1
                        && this._sequenceNum != _width * _height
                        && this._sequenceNum != 0) {
                    releaseGroup(this.group());
                }
                if (s1._sequenceNum != 1 && s1._sequenceNum != _width * _height
                        && s1._sequenceNum != 0) {
                    this._sequenceNum = s1._sequenceNum - 1;
        ...
        /** Disconnects me from my current successor, if any.*/
        void disconnect() {
            Sq next = _successor;
            if (next == null) {
                return;
            ...
            if (_sequenceNum == 0) {
                if (this._successor == null && this._predecessor == null
                ...
                if (this._predecessor != null) {
                    if (!this._hasFixedNum && !this._predecessor._hasFixedNum) {
                        this._sequenceNum = 0;
                        this._predecessor._sequenceNum = 0;
                        this._group = newGroup();
                        this._predecessor._group = this._group;
                    }
                } else {
                    this._group = -1;
                }
                if (next.successor()  != null) {
                    if (!next._hasFixedNum && !next._successor._hasFixedNum) {
                        next._sequenceNum = 0;
                        next._successor._sequenceNum = 0;
                        next._group = newGroup();
                        next._successor._group = this._group;
                    }
                } else {
                    next._group = -1;
                }
                for (Sq square: _allSquares) {
                    if (square._group == next._group) {
                        square._head = next;
                    }
                }
            }
