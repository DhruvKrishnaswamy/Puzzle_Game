...
class Place {
    ...
    /**
     * Getting x,y coordinates for the squares by looping
     * through the squares. We then create a new PlaceList
     * and add a place for a given direction
     */
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                for (int k = 0; k < 9; k++) {
                    int tempx = i, tempy = j;
                    M[i][j][k] = new PlaceList();
                    if ((tempx + dx(k) <= width) && (tempy + dy(k) <= height)) {
                        Place p = pl(tempx, tempy);
                        M[i][j][k].add(p);
                        M[i][j][0].add(p);
                    }
                }
